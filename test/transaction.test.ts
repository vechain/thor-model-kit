...
describe("transaction", () => {
    ...
    it('unsigned', () => {
        ...
        expect(unsigned.id).deep.equal(new Bytes32(new Buffer(0)))
    ...
    it('invalid body', () => {
        ...
        expect(() => { new Transaction({ ...body, blockRef: new Buffer(0) }) }).to.throw()
        expect(() => { new Transaction({ ...body, blockRef: new Buffer(9) }) }).to.throw()
    ...
    it("signed", () => {
        expect(signed.signature).deep.equal(new Buffer('f76f3c91a834165872aa9464fc55b03a13f46ea8d3b858e528fcceaf371ad6884193c3f313ff8effbb57fe4d1adc13dceb933bedbf9dbb528d2936203d5511df00', 'hex'))
    ...
    incorrectlySigned.signature = new Buffer([1, 2, 3])
    it('incorrectly signed', () => {
        expect(() => { incorrectlySigned.signer }).to.throw()
        expect(incorrectlySigned.id).deep.equal(new Bytes32(new Buffer(0)))
    })
})
